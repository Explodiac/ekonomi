...
const IC = 'cat-interest'
function tx(amount: number, cash_date: string, category_id = IC): InsightTransaction {
    return { amount, type: 'expense', cash_date, category_id }
}

test('faiz artışı → 95 öncelikli, iki ayı karşılaştıran cümle', () => {
    const txs = [tx(2100, '2026-08-10'), tx(3200, '2026-09-05')]
    const r = ruleInterestIncrease(txs, '2026-09', IC)
    assert.ok(r)
...
test('ilk kez faiz (geçen ay 0) → 92 öncelikli', () => {
    const r = ruleInterestIncrease([tx(1500, '2026-09-05')], '2026-09', IC)
    assert.ok(r)
...
test('faiz düşmüş/sabit → uyarı yok', () => {
    ...
    assert.equal(ruleInterestIncrease(txs, '2026-09', IC), null)
})

test('bu ay faiz yok → null', () => {
    assert.equal(ruleInterestIncrease([tx(2000, '2026-08-10')], '2026-09', IC), null)
})

test('faiz kategorisi verilmemişse → null', () => {
    assert.equal(ruleInterestIncrease([tx(2000, '2026-09-05')], '2026-09', undefined), null)
})
